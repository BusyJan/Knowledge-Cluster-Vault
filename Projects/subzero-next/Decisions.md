...
## 2026-04-19 — atopile als strategische PCB-Workflow-Plattform (Trial)

- **Context:** Aktueller KiCad-Workflow ist langsam — manuelle Schematic-Klicks, Hand-Patches in Python, kein semantisches Verständnis durch AI, keine Reuse zwischen Iterationen. User fragt nach "breakthrough" und Skill/Add-on das auch in Zukunft funktioniert. Recherche: atopile (https://atopile.io, 3.1k stars, native Cursor/VSCode Extension) ist Hardware-as-Code mit Python-DSL, generiert KiCad-PCBs.
- **Decision:** atopile als **Trial-Abo** etabliert: installiert via uv (v0.15.6), Live-Test mit 50-Zeilen Decoupling+Reset+LED Cluster bewies 5-10× Speedup gegenüber KiCad-Klicks (16s build inkl Auto-BOM mit echten LCSC-Parts und Datasheet-Downloads). **Cursor-Skill `pcb-engineer`** als globaler Knowledge-Hub erstellt (~/.cursor/skills/pcb-engineer/) — funktioniert sowohl für KiCad-Patches (existing SubZero) als auch atopile (zukünftige Projekte). **Component-DB Pattern** als single source of truth für Part-Daten (Höhe, LCSC, Stock).
- **Consequences:** SubZero v1 wird **in KiCad** fertiggestellt (zu weit für Migration). Alle NEUEN Hardware-Projekte starten als atopile-Projekt. Skill ist global verfügbar, läuft automatisch bei "PCB"/"KiCad"/"atopile" Triggern in jedem Cursor-Workspace. Trade-off: atopile package registry hat noch Lücken (atopile/esp32s3 outdated), aber Standard Library deckt 95% der Generic-Components ab. Custom Components müssen via `ato create part` als atomic components definiert werden — einmal gemacht, dann reusable. Trial bewertet nach 2-3 atopile-Projekten (≥1 Monat Erfahrung).
